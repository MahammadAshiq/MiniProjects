...
async function searchCity() {
  ...
  try {
    ...
    // Attempt 2: Nominatim/OpenStreetMap (covers villages, hamlets, localities)
    ...
    const placeName = addr.village || addr.hamlet || addr.town || addr.suburb || addr.city_district || p.display_name.split(',')[0];
    const state = addr.state || '';
    const country = addr.country || '';
    const label = [placeName, state, country].filter(Boolean).join(', ');

    await loadWeatherByCoords(parseFloat(p.lat), parseFloat(p.lon), label);
  } catch (err) {
    console.error(err);
